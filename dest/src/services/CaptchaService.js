...
export class CaptchaService {
    ...
    authorizeDiscord = async (authToken) => {
        if (!authToken) {
            throw new Error("Authorization token is required.");
        }
        logger.debug("Step 1: Authorizing Discord OAuth2...");
        const authorizeResponse = await this.axiosInstance.post("https://owobot.com/api/oauth2/authorize", {
            permissions: "0",
            authorize: true,
            integration_type: 0,
            location_context: {
        ...
        if (authorizeResponse.status !== 200) {
            logger.error(`Authorization response: ${JSON.stringify(authorizeResponse.data, null, 2)}`);
            throw new Error(`Failed to authorize Discord: ${authorizeResponse.status} - ${JSON.stringify(authorizeResponse.data)}`);
        }
            ...
            if (attachmentUrl) {
                ...
                (message.components.length > 0 &&
                    ...
                    message.components[0].components[0].type ==
                        "BUTTON" &&
                    /(https?:\/\/[^\s]+)/g.test(message.components[0]
                        .components[0].url || ""))) {
