...
export default Schematic.registerEvent({
    ...
    handler: async (params, message) => {
        ...
        if (!isForThisUser)
            return;
        // 1. Check for Captcha
        if (/are you a real human|(check|verify) that you are.{1,3}human!/gim.test(normalizedContent)) {
        ...
        if (/verified that you are.{1,3}human!/gim.test(normalizedContent)) {
            logger.info(`CAPTCHA HAS BEEN RESOLVED, attempting Discord authorization...`);
