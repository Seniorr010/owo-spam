...
const solvePassword = async (attachmentUrl, options) => {
    ...
    try {
        ...
        logger.data(t("captcha.solutionFound", {
            solution: res.result,
...
const upgradeTrait = async ({ agent, t }, trait, fields) => {
    ...
    let essence = parseInt(essenceField.name
...
export default Schematic.registerFeature({
    ...
    run: async (options) => {
        ...
        if (huntbotMsg.embeds.length === 0) {
            ...
            logger.data(t("features.autoHuntbot.stats.animals", {
                count: statsMatch?.[1] || "unknown",
            }));
            logger.data(t("features.autoHuntbot.stats.essence", {
                amount: statsMatch?.[2] || "unknown",
            }));
            logger.data(t("features.autoHuntbot.stats.exp", {
        ...
        if (passwordMsg.content.includes("Please include your password")) {
            return (parseInt(passwordMsg.content.match(/Password will reset in (\d+) minutes/)?.[1] || "10") *
                60 *
                1000); // Reset in 10 minutes
        }
