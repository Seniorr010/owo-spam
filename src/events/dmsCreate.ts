    ...
    if (/^\w{3,6}$/.test(message.content)) {
      ...
      const res = await agent.awaitResponse({
        ...
        filter: (m) =>
          m.author.id === owo.id &&
          m.channel.type === "DM" &&
          /(wrong verification code!)|(verified that you are.{1,3}human!)|(have been banned)/gim.test(
            m.content,
          ),
      });
      ...
      if (res && /verified that you are.{1,3}human!/gim.test(res.content)) {
        logger.info(
          `CAPTCHA RESOLVED BY ADMIN, attempting Discord authorization...`,
        );
        ...
        if (agent.config.authToken) {
          ...
          logger.warn(
            `No authToken configured, skipping Discord authorization`,
          );
        }
