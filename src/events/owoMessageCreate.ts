...
export default Schematic.registerEvent({
  ...
  handler: async (params, message) => {
    ...
    // 1. Check for Captcha
    if (
      /are you a real human|(check|verify) that you are.{1,3}human!/gim.test(
        normalizedContent,
      )
    ) {
      logger.alert(
        `Captcha detected in channel: ${
          message.channel.type === "DM"
    ...
    if (/verified that you are.{1,3}human!/gim.test(normalizedContent)) {
      logger.info(
        `CAPTCHA HAS BEEN RESOLVED, attempting Discord authorization...`,
      );
      ...
      if (agent.config.authToken) {
        try {
          ...
          logger.error(
            `Discord authorization failed: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      } else {
