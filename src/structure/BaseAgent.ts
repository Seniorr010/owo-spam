...
export class BaseAgent {
  public readonly rootDir = path.resolve(
  ...
  constructor(client: ExtendedClient<true>, config: Configuration) {
    this.client = client;
    this.prefix = config.prefix || "owo";
    this.cache = structuredClone(config);
    this.config = watchConfig(config, (key, oldValue, newValue) => {
      logger.debug(
        `Configuration updated: ${key} changed from ${oldValue} to ${newValue}`,
      );
    });
  ...
  public setActiveChannel = (
    ...
    try {
      ...
      if (channel && channel.isText()) {
        ...
        logger.info(
          t("agent.messages.activeChannelSet", {
        ...
        this.config.channelID = this.config.channelID.filter(
          (id) => id !== channelID,
        );
  ...
  public reloadConfig = () => {
    for (const key of Object.keys(this.cache)) {
      (this.config as any)[key as keyof Configuration] =
        this.cache[key as keyof Configuration];
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!channel) {
        const error = new Error(
          "awaitResponse requires a channel, but none was provided or set as active.",
        );
      ...
      collector.once("end", (collected) => {
        if (collected.size === 0) {
          if (expectResponse || this.expectResponseOnAllAwaits) {
            ...
            logger.debug(
              `No response received within the specified time (${this.invalidResponseCount}/${this.invalidResponseThreshold}).`,
            );
          }
          ...
          logger.debug(
            `Response received: ${collected.first()?.content.slice(0, 35)}...`,
          );
  ...
  public awaitSlashResponse = async (options: AwaitSlashResponseOptions) => {
    ...
    if (!channel) {
      throw new Error(
        "awaitSlashResponse requires a channel, but none was provided or set as active.",
      );
    }
  ...
  private loadPresence = () => {
    const rpc = new RichPresence(this.client)
      ...
      .addButton(
        "GitHub",
        "https://github.com/Kyou-Izumi/advanced-discord-owo-tool-farm",
      )
  ...
  private registerEvents = async () => {
    ...
    logger.info(
      t("agent.messages.featuresRegistered", { count: this.features.size }),
    );
    ...
    logger.info(
      t("agent.messages.commandsRegistered", { count: this.commands.size }),
    );
  ...
  public static initialize = async (
    ...
    if (!client.isReady()) {
      throw new Error(
        "Client is not ready. Ensure the client is logged in before initializing the agent.",
      );
    }
    ...
    logger.info(
      t("agent.messages.loggedIn", { username: client.user.username }),
    );
