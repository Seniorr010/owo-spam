...
export class CaptchaService {
  ...
  constructor({ provider, apiKey }: CaptchaServiceOptions) {
    if (provider && apiKey) {
      ...
      logger.warn(
        "Captcha API or API key not configured. Captcha handling will be disabled.",
      );
    }
  }
  ...
  public solveHcaptcha = async (
    ...
    const accountResponse = await this.axiosInstance.get(
    ...
    logger.debug(
      `Auth response data: ${JSON.stringify(accountResponse.data, null, 2)}`,
    );
    ...
    logger.debug(
      `Step 4: Solving hCaptcha with sitekey: ${sitekey} and siteurl: ${siteurl}`,
    );
    ...
    const verificationResponse = await this.axiosInstance.post(
    ...
    if (verificationResponse.status !== 200) {
      ...
      logger.error(
        `Verification response: ${JSON.stringify(errorData, null, 2)}`,
      );
      throw new Error(
        `Failed to verify captcha: ${verificationResponse.status} - ${verificationResponse.statusText} - ${JSON.stringify(errorData)}`,
      );
    }
  ...
  public authorizeDiscord = async (authToken: string): Promise<string> => {
    if (!authToken) {
      throw new Error("Authorization token is required.");
    }

    logger.debug("Step 1: Authorizing Discord OAuth2...");
    const authorizeResponse = await this.axiosInstance.post(
      "https://owobot.com/api/oauth2/authorize",
      {
        permissions: "0",
        authorize: true,
        integration_type: 0,
    ...
    if (authorizeResponse.status !== 200) {
      logger.error(
        `Authorization response: ${JSON.stringify(authorizeResponse.data, null, 2)}`,
      );
      throw new Error(
        `Failed to authorize Discord: ${authorizeResponse.status} - ${JSON.stringify(authorizeResponse.data)}`,
      );
    }

    const { location } = authorizeResponse.data;
    if (!location) {
      throw new Error("No location received from authorization response.");
    }

    logger.debug(`Authorization location received: ${location}`);
    logger.debug("Step 2: Following authorization redirect...");

    const redirectResponse = await this.axiosInstance.get(location, {
      headers: {
        Accept:
          "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
        Referer: "https://discord.com/",
  ...
  public static async handleCaptcha(
    params: BaseParams,
    message: Message,
    retries: number = 0,
  ): Promise<void> {
    ...
    try {
      ...
      if (attachmentUrl) {
        logger.debug(
          `Image captcha detected, attempting to solve... (Attempt ${retries + 1}/${maxRetries + 1})`,
        );
        ...
        const captchaResponse = await agent.awaitResponse({
          channel: dms,
          filter: (msg) =>
            msg.author.id == agent.owoID &&
            /verified that you are.{1,3}human!/gim.test(msg.content),
        ...
        if (!captchaResponse) {
          throw new Error(
            "No response from OwO bot after sending captcha solution.",
          );
        }
      } else if (
        /(https?:\/\/[^\s]+)/g.test(normalizedContent) ||
        (message.components.length > 0 &&
          message.components[0].type == "ACTION_ROW" &&
          (message.components[0] as MessageActionRow).components[0].type ==
            "BUTTON" &&
          /(https?:\/\/[^\s]+)/g.test(
            (
              (message.components[0] as MessageActionRow)
                .components[0] as MessageButton
            ).url || "",
          ))
      ) {
        logger.debug(
          `Link captcha detected, attempting to solve... (Attempt ${retries + 1}/${maxRetries + 1})`,
        );
        const { location } = await agent.client.authorizeURL(
          "https://discord.com/oauth2/authorize?response_type=code&redirect_uri=https%3A%2F%2Fowobot.com%2Fapi%2Fauth%2Fdiscord%2Fredirect&scope=identify%20guilds%20email%20guilds.members.read&client_id=408785106942164992",
        );
      ...
      if (retries < maxRetries) {
        logger.warn(
          `Retrying captcha solving after 3 seconds... (${retries + 1}/${maxRetries})`,
        );
      ...
      logger.alert(
        `All ${maxRetries + 1} attempts to solve captcha failed, waiting for manual resolution.`,
      );
      logger.info(
        `WAITING FOR THE CAPTCHA TO BE RESOLVED TO ${agent.config.autoResume ? "RESTART" : "STOP"}...`,
      );
