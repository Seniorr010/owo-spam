...
const solvePassword = async (
  attachmentUrl: string,
  options: SolvePasswordOptions,
  ...
  if ("provider" in options && "apiKey" in options) {
    ...
    if (!isValidResult) {
      logger.warn(
        t("features.autoHuntbot.errors.invalidCaptchaResult", { result }),
      );
      return undefined;
    }
...
const upgradeTrait = async (
  ...
  let essence = parseInt(
    essenceField.name
      .match(/Animal Essence - `([\d,]+)`/i)?.[1]
      .replace(/,/g, "") || "0",
  );
  ...
  logger.data(
    t("features.autoTrait.essenceStatus", {
      trait,
...
export default Schematic.registerFeature({
  ...
  run: async (options) => {
    ...
    const huntbotMsg = await agent.awaitResponse({
      ...
      filter: (m) =>
        m.author.id === agent.owoID &&
        (m.content.includes("BEEP BOOP. I AM BACK") ||
          (m.embeds.length > 0 &&
            m.embeds[0].author !== null &&
            m.embeds[0].author.name.includes(
              m.guild?.members.me?.displayName!,
            ) &&
    ...
    if (huntbotMsg.embeds.length === 0) {
      ...
      logger.data(
        t("features.autoHuntbot.stats.animals", {
          count: statsMatch?.[1] || "unknown",
        }),
      );
      logger.data(
        t("features.autoHuntbot.stats.essence", {
          amount: statsMatch?.[2] || "unknown",
        }),
      );
      logger.data(
        t("features.autoHuntbot.stats.exp", {
          amount: statsMatch?.[3] || "unknown",
        }),
      );
    ...
    if (
      fields[fields.length - 1].name.includes("HUNTBOT is currently hunting!")
    ) {
      ...
      if (match) {
        return (
          parseInt(match[2] || "0") * 60 * 60 * 1000 +
          parseInt(match[3]) * 60 * 1000 +
          ranInt(0, 5 * 60 * 1000)
        ); // Add random upto 5 mins
      }
    ...
    if (passwordMsg.content.includes("Please include your password")) {
      return (
        parseInt(
          passwordMsg.content.match(
            /Password will reset in (\d+) minutes/,
          )?.[1] || "10",
        ) *
        60 *
        1000
    ...
    if (passwordMsg.content.includes("I AM STILL HUNTING")) {
      ...
      return (
        hours * 60 * 60 * 1000 + minutes * 60 * 1000 + ranInt(0, 5 * 60 * 1000)
      ); // Add random upto 5 mins
    }
    ...
    if (
      agent.config.captchaAPI &&
      agent.config.apiKey &&
      !agent.config.useAdotfAPI
    ) {
