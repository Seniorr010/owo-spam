...
export const ConfigSchema = z
  .object({
    ...
    wayNotify: z
      .array(z.enum(["webhook", "dms", "call", "music", "popup"]))
      .default([]),
    ...
    autoTrait: z
      .enum(["efficiency", "duration", "cost", "gain", "experience", "radar"])
      .optional(),
    ...
    gemTier: z
      ...
      .default(["common", "uncommon", "rare", "epic", "mythical"])
      .optional(),
    ...
    autoQuote: z.array(z.enum(["owo", "quote"])).default(["owo"]),
    autoRPP: z
      .array(z.enum(["run", "pup", "piku"]))
  ...
  .check(({ issues, value }) => {
    if (value.wayNotify.includes("webhook") && !value.webhookURL) {
      issues.push({
        ...
        message:
          "Webhook URL is required when 'webhook' is selected in wayNotify",
      });
    }
    if (
      (value.wayNotify.includes("dms") || value.wayNotify.includes("call")) &&
      !value.adminID
    ) {
      issues.push({
        code: "custom",
        input: value.adminID,
        message:
          "Admin ID is required when 'dms' or 'call' is selected in wayNotify",
      });
    }
