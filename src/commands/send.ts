...
export default Schematic.registerCommand({
  ...
  execute: async ({ agent, message, t, args }) => {
    ...
    const guildMember = message.guild.members.cache.get(
      message.mentions.users.first()?.id || user.replace(/<@!?(\d+)>/, "$1"),
    );
    if (!guildMember) {
      return message.reply({
        content: t("commands.send.invalidUser"),
    ...
    try {
      const response = await agent.awaitResponse({
        trigger: () =>
          agent.send(`${agent.config.prefix} give ${user} ${amount}`),
        filter: (msg) =>
          ...
          (msg.components[0] as MessageActionRow).components[0].type ===
            "BUTTON",
        time: 15000,
      });
      ...
      const button = (response.components[0] as MessageActionRow)
